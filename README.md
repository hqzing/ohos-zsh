...
鸿蒙 PC 内置了 zsh，系统自带的“终端”（HiShell）里面默认的 shell 就是 zsh，一般情况下我们不需要再自己另带一个进去用。

如果你一定要带进去用，那也是可以的，只是操作比较繁琐。
...
这个 zsh 静态链接了它所依赖的库（libc 除外），并集成了 terminfo 数据库到自己的二进制中，因此支持单文件使用。
...
如果这里的值也不能满足（极小概率，一些非常深度的使用场景可能遇到），你需要自己另外弄一套 terminfo 数据库到设备上，再根据自己的实际情况配置 TERM 和 TERMINFO 环境变量。
